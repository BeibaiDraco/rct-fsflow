...
for ALL sessions and BOTH alignments (stim, sacc).

Pipeline per align ∈ {stim, sacc}, per session sid, per orientation ∈ {vertical, horizontal, pooled}:
  1) train_axes.py
  2) qc_axes.py
  3) For each feature:
       - stim: C and R
...
Flow tags include align and orientation to avoid overwriting:
  ...
  --null_methods trial_shuffle \
  --standardize_modes none zscore_regressors
...
def main():
    ap = argparse.ArgumentParser(
        description="Run axes/QC/flow/overlays for all sessions, "
    ...
    for sid in sids_stim:
        print(f"\n[align=stim] Session {sid}")
        for ori in orientations:
            print(f"\n[block] ori={ori}")

            # Orientation-specific axes tag (per align)
            axes_tag = f"{args.axes_tag_base}-stim-{ori}"
            ...
            for feat in feats:
                for std_mode in std_modes:
                    for null_method in null_methods:
                        ...
                        flow_tag_base_align_ori = f"{args.flow_tag_base}-stim-{ori}"
                        flow_tag = f"{flow_tag_base_align_ori}-{std_short}-{null_short}"

                        print(f"\n[combo] sid={sid}, align=stim, ori={ori}, feat={feat}, "
                              f"std={std_mode}, null={null_method} → {flow_tag}")
                        ...
                        if not args.no_pt_filter:
                            flow_cmd += ["--pt_min_ms", str(args.pt_min_ms_stim)]
    ...
    for sid in sids_sacc:
        print(f"\n[align=sacc] Session {sid}")
        for ori in orientations:
            print(f"\n[block] ori={ori}")

            axes_tag = f"{args.axes_tag_base}-sacc-{ori}"
            ...
            # For sacc we follow your original pipeline: feature S only
            feats = ["S"]
            for feat in feats:
                for std_mode in std_modes:
                    for null_method in null_methods:
                        std_short = std_short_map[std_mode]
                        null_short = null_short_map[null_method]
                        flow_tag_base_align_ori = f"{args.flow_tag_base}-sacc-{ori}"
                        flow_tag = f"{flow_tag_base_align_ori}-{std_short}-{null_short}"

                        print(f"\n[combo] sid={sid}, align=sacc, ori={ori}, feat={feat}, "
                              f"std={std_mode}, null={null_method} → {flow_tag}")
                        ...
                        flow_cmd = [args.python, CLI_FLOW,
                                    ...
                                    "--standardize_mode", std_mode]
                        if not args.no_pt_filter:
                            flow_cmd += ["--pt_min_ms", str(args.pt_min_ms_sacc)]
                        rc = run(flow_cmd, dry=args.dry_run, continue_on_error=args.continue_on_error)
                        if rc and not args.continue_on_error:
                            return
    ...
    print("\n[done] All sessions completed for all alignments, orientations, null_methods, and standardize_modes.")
