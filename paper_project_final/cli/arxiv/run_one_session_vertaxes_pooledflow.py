#!/usr/bin/env python3
"""
Run axes (vertical-only), QC (vertical), and flow (pooled trials)
for ONE session (both stim & sacc, if caches exist).

For each align in {stim, sacc}:

  1) train_axes.py         (orientation=vertical)
  2) qc_axes.py            (orientation=vertical)
  3) flow_session.py       (orientation=pooled, but axes_tag from vertical training)
...
So these are distinct from your previous “orientation=vertical/pooled/horizontal” runs.
"""
...
def main():
    ap = argparse.ArgumentParser(
        description="Vertical-only axes, pooled-trial flow, for one session (stim & sacc)."
    )
    ...
    ap.add_argument("--axes_tag_base", default="axes_vertTrain",
                    help="Base tag for vertical-trained axes; "
                         "final tag = <axes_tag_base>-<align>-vertical (default: axes_vertTrain)")
    ap.add_argument("--flow_tag_base", default="crvertTrainPooled",
                    help="Base for pooled-flow tags; "
                         "final base = <flow_tag_base>-<align>-pooled (default: crvertTrainPooled)")
    ...
    for align in aligns:
        ...
        train_ori = "vertical"
        qc_ori = "vertical"
        flow_ori = "pooled"  # pooled trials (orientation=None in flow)

        axes_tag = f"{args.axes_tag_base}-{align}-{train_ori}"

        # --- 1) train_axes (vertical only) ---
        ...
        for feat in feats:
            for std_mode in std_modes:
                for null_method in null_methods:
                    ...
                    # --- 3) flow_session (pooled trials, vertical axes) ---
    ...
    print(f"\n[done] SID={args.sid} vertical-axes + pooled-flow sweep complete.")
