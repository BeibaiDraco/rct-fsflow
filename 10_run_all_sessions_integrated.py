...
def main():
    ...
    # 03: cache (vertical-only) — reuse if meta matches
    sh(sys.executable, BASE/"03_cache_binned.py",
       "--root", root, "--session", sid,
       "--bin", 0.010, "--t0", -0.25, "--t1", 0.80,
       "--targets_vert_only", "--reuse_cache")

    # 04: subspaces (per-category center/whiten for R) — write under tag, skip if exists
    ...
    # 08i: time-sliding + integrated band test (all pairs) — use same tag, skip if exists
