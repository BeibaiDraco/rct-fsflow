...
# -*- coding: utf-8 -*-
"""
16_make_session_overlays_noninduced.py

Create per-session bidirectional overlays (A→B and B→A) inside each non-induced tag folder.
- No null bands; plot only null mean lines (thin) for each direction.
- Annotate per-time p-values next to the computed curves (downsampled).
- Works for RAW and/or INT, and for C and/or R.

Example:
  python 16_make_session_overlays_noninduced.py \
      --tag win160_k5_perm500_integrated --mode both --labels C R \
...
def load_noninduced_npz(path: Path):
    ...
    rec = {
        ...
        "C_p_raw": z.get("C_p_raw", None), "R_p_raw": z.get("R_p_raw", None),
        # INT
...
def main():
    ...
    ap.add_argument("--tag", required=True, help="non-induced tag folder ending with 'integrated'")
    ...
    for sdir in sorted(SESS_ROOT.glob("*")):
        ...
        # gather pairs present in both directions
        files = list(tag_dir.glob("flow_timeseriesINT_*to*.npz"))
        pairs = {}
        for f in files:
            name = f.stem[len("flow_timeseriesINT_"):]
        ...
        for (A,B), d in sorted(pairs.items()):
            if f"{A}->{B}" not in d or f"{B}->{A}" not in d:
                continue
            z_ab = load_noninduced_npz(d[f"{A}->{B}"])
            z_ba = load_noninduced_npz(d[f"{B}->{A}"])

            # CATEGORY
            if "C" in args.labels:
                # choose reference axis (prefer C time axis from AB)
