    ...
    for (A, B) in pairs:
        ...
        try:
            res = compute_flow_timecourse_for_pair(
                ...
                perm_within=args.perm_within,                           # NEW
            )
        except ValueError as e:
            print(f"[skip] {A}->{B}: {e}")
            continue

        out_dir = os.path.join(args.out_root, args.align, args.sid, "flow", args.tag, args.feature)
        os.makedirs(out_dir, exist_ok=True)
        np.savez_compressed(os.path.join(out_dir, f"flow_{args.feature}_{A}to{B}.npz"), **res)
        print(f"[{args.sid}][{A}->{B}] wrote {os.path.join(out_dir, f'flow_{args.feature}_{A}to{B}.npz')}")
