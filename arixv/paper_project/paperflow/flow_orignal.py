# paper_project/paperflow/flow.py
...
def compute_flow_timecourse_for_pair(
    ...
    seed: int = 0,
    perm_within: str = "CR",        # NEW: 'CR' (default), 'other', 'C', 'R', 'none'
) -> Dict[str, np.ndarray]:
    ...
    # meta in caches is a json string; decode safely
    metaA = cacheA["meta"]
    ...
    for b in range(start_b, Bbins):
        ...
        if include_B_lags and XB_lags.shape[2] > 0:
            X_red = np.concatenate([X_red, XB_lags[:, b, :]], axis=1)  # (N, 1 + L*K_B)
        sse_red = _ridge_sse_multi(Yt, X_red, ridge)
        # full adds A lags
        X_full = np.concatenate([X_red, XA_lags[:, b, :]], axis=1)    # (N, 1 + L*K_B + L*K_A)
    ...
    if perms > 0:
        ...
        # choose permutation strata independently of 'induced' labels
        if perm_within == "CR":
            ...
            else:  # S has no "other" feature → use CR
                perm_labels = _encode_joint_labels(cacheA, mask, ("lab_C", "lab_R"))
        elif perm_within == "C":
            perm_labels = _label_vec(cacheA, "lab_C", mask)
        elif perm_within == "R":
            ...
            perm_labels = np.full(int(mask.sum()), np.nan)
        else:
            raise ValueError(f"Unknown perm_within='{perm_within}'")

        strata = perm_labels if perm_labels is not None else np.full(N, np.nan)

        for p in range(P):
            # Two independent shuffles: A for A->B null, B for B->A null
            perm_idx_A = _permute_within_strata(strata, rng)
            perm_idx_B = _permute_within_strata(strata, rng)

            YA_perm = YA[perm_idx_A]
            YB_perm = YB[perm_idx_B]

            XA_lags_perm = _build_lag_stack_multi(YA_perm, L)
            XB_lags_perm = _build_lag_stack_multi(YB_perm, L)

            vals_A = np.full(Bbins - start_b, np.nan)
            vals_B = np.full(Bbins - start_b, np.nan)

            for i, b in enumerate(range(start_b, Bbins)):
                # ---------- A -> B null (permute source A) ----------
                Yt = YB[:, b, :]
                X_red = ones
                if include_B_lags and XB_lags.shape[2] > 0:
                    # reduced keeps B-lags (UNpermuted)
                    X_red = np.concatenate([X_red, XB_lags[:, b, :]], axis=1)
                sse_red = _ridge_sse_multi(Yt, X_red, ridge)

                # full adds permuted A-lags
                X_full = np.concatenate([X_red, XA_lags_perm[:, b, :]], axis=1)
                sse_full = _ridge_sse_multi(Yt, X_full, ridge)
                if sse_full > 0 and sse_red > 0:
                    vals_A[i] = 0.5 * N * np.log2(sse_red / sse_full)

                # ---------- B -> A null (permute source B) ----------
                Yt_rev = YA[:, b, :]
                X_red_r = ones
                if include_B_lags and XA_lags.shape[2] > 0:
                    # reduced keeps A-lags (UNpermuted)
                    X_red_r = np.concatenate([X_red_r, XA_lags[:, b, :]], axis=1)
                sse_red_r = _ridge_sse_multi(Yt_rev, X_red_r, ridge)

                # full adds PERMUTED B-lags
                X_full_r = np.concatenate([X_red_r, XB_lags_perm[:, b, :]], axis=1)
                sse_full_r = _ridge_sse_multi(Yt_rev, X_full_r, ridge)
                if sse_full_r > 0 and sse_red_r > 0:
        ...
        # null summaries
        null_mean = np.nanmean(all_A, axis=0); null_std = np.nanstd(all_A, axis=0, ddof=1)
        null_mean_r = np.nanmean(all_B, axis=0); null_std_r = np.nanstd(all_B, axis=0, ddof=1)
        obsA = bits_AtoB[start_b:]; obsB = bits_BtoA[start_b:]
        validA = np.isfinite(obsA)[None, :]
        validB = np.isfinite(obsB)[None, :]
        with np.errstate(invalid="ignore"):
            # only compute where obs is finite; else set NaN
            # continuity correction: p = (1 + #null >= obs) / (1 + P)
            pA_full = np.full(obsA.shape, np.nan)
            pB_full = np.full(obsB.shape, np.nan)
            if np.any(validA):
                counts_A = np.sum(all_A[:, validA[0]] >= obsA[validA[0]], axis=0)
                pA_full[validA[0]] = (1 + counts_A) / (1 + P)
            if np.any(validB):
                counts_B = np.sum(all_B[:, validB[0]] >= obsB[validB[0]], axis=0)
                pB_full[validB[0]] = (1 + counts_B) / (1 + P)
    ...
    return dict(
        ...
        meta=dict(
            ...
            perm_within=str(perm_within),                     # NEW
