# paper_project/paperflow/flow.py — FIXED VERSION
...
def _encode_joint_labels(cache: Dict, mask: np.ndarray, keys: Tuple[str, ...]) -> Optional[np.ndarray]:
    ...
    for k in keys:
        ...
        if v is None:
            return None
        # robust integer coding (handle floats cleanly)
        vecs.append(np.asarray(np.round(v), dtype=int))
    # mixed-radix: concatenate along digits with a large base
...
def _permute_within_strata(labels: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    ...
    # Count how many valid strata we have
    if not np.any(good):
        # No valid labels - shuffle everything together
...
def _circular_shift_time(Y: np.ndarray, rng: np.random.Generator, min_shift: int = 1) -> np.ndarray:
    ...
    This breaks the temporal alignment between A and B while preserving
    within-trial autocorrelation structure.
    """
    N, B, K = Y.shape
    Y_shifted = np.empty_like(Y)
    for n in range(N):
        shift = rng.integers(min_shift, B)  # random shift per trial
...
def _phase_randomize(Y: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    ...
    This is a more aggressive null that breaks temporal coherence.
    """
    N, B, K = Y.shape
    ...
    for n in range(N):
        for k in range(K):
            ...
            # Randomize phases, preserve magnitudes
            phases = rng.uniform(0, 2*np.pi, len(fft_vals))
            phases[0] = 0  # Keep DC component real
            if B % 2 == 0:
                phases[-1] = 0  # Nyquist component real for even N
...
def compute_flow_timecourse_for_pair(
    ...
    null_method: str = "trial_shuffle",  # NEW: 'trial_shuffle', 'circular_shift', 'phase_randomize'
) -> Dict[str, np.ndarray]:
    """
    ...
    ----------
    null_method : str
        Method for constructing the null distribution:
        - 'trial_shuffle': Shuffle trial indices within strata (default, preserves within-trial autocorrelation)
        - 'circular_shift': Circular time shift per trial (breaks temporal alignment, preserves autocorrelation)
        - 'phase_randomize': Randomize phases in frequency domain (most aggressive, breaks temporal coherence)
    
    Notes on null interpretation
    ----------------------------
    The null distribution answers: "What ΔLL would we expect if there were no true transfer from A to B?"
    
    - trial_shuffle: Tests if trial-by-trial correspondence matters beyond condition structure.
      May OVERESTIMATE null if trials have similar temporal patterns.
    
    - circular_shift: Tests if precise temporal alignment matters. 
      More conservative (lower null), better for detecting weak effects.
    
    - phase_randomize: Tests if any temporal structure matters.
      Most conservative, may be too aggressive for some applications.
    """
    ...
    if induced:
        if feature == "C":
            ...
            labs_induce = _encode_joint_labels(cacheA, mask, ("lab_C", "lab_R"))  # remove both C & R
    ...
    if perms > 0:
        ...
        # CRITICAL FIX: Handle None/all-NaN labels
        if perm_labels is None:
            # No stratification requested - use single stratum (shuffle all together)
            strata = np.zeros(N, dtype=float)  # All same stratum
            ...
            if n_valid == 0:
                # All labels are NaN - this is a problem!
                warnings.warn(f"All permutation labels are NaN! Shuffling across all trials.")
        ...
        for p in range(P):
            ...
            if null_method == "trial_shuffle":
                ...
                # Circular time shift - breaks temporal alignment, preserves autocorrelation
                ...
                # Phase randomization - breaks temporal coherence entirely
            ...
            # Build lag stacks from null data
            ...
            for i, b in enumerate(range(start_b, Bbins)):
                ...
                Yt = YB[:, b, :]  # Original target (not permuted)
                
                # Use cached reduced model SSE
                sse_red = sse_red_cache[b]

                # Full model with null A-lags
                X_red = ones
                if include_B_lags and XB_lags.shape[2] > 0:
                    X_red = np.concatenate([X_red, XB_lags[:, b, :]], axis=1)
                ...
                Yt_rev = YA[:, b, :]  # Original target (not permuted)
                
                # Use cached reduced model SSE
                sse_red_r = sse_red_rev_cache[b]

                # Full model with null B-lags
                X_red_r = ones
                if include_B_lags and XA_lags.shape[2] > 0:
                    X_red_r = np.concatenate([X_red_r, XA_lags[:, b, :]], axis=1)
        ...
        # Warn if no actual shuffling occurred
    ...
    return dict(
        ...
        meta=dict(
            ...
            null_method=str(null_method),  # NEW
