...
- Filters to RCT-correct with Align_to_cat_stim_on
...
- Saves: results/caches/<sid>_<AREA>.npz with:
    X (counts int16), meta (json str), trials (pandas JSON)
Works with actual area folders for both monkeys:
    MFEF/MLIP/MSC and SFEF/SLIP/SSC
If --areas is omitted, auto-detects valid areas present in that session.
"""
...
def reparam_CR(df: pd.DataFrame) -> pd.DataFrame:
...
def main():
    ...
    ap.add_argument(
        ...
        help="Actual area folders to cache (e.g., MFEF MLIP MSC or SFEF SLIP SSC). "
             "If omitted, caches whichever of {MFEF,MLIP,MSC,SFEF,SLIP,SSC} are present in the session."
    )
    ap.add_argument("--bin", type=float, default=0.010, help="Bin size in seconds")
    ap.add_argument("--t0", type=float, default=-0.3, help="Window start (s)")
    ap.add_argument("--t1", type=float, default=1.2,  help="Window end (s)")
    args = ap.parse_args()
    ...
    # Trials & labels
    df = reparam_CR(trials_table(sdir))
