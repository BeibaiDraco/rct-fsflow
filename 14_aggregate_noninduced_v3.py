...
# -*- coding: utf-8 -*-
"""
14_aggregate_noninduced_v3.py

Group analysis for NON-INDUCED results using per-session permutation p-values + meta-analysis.
Also produces bidirectional overlays (A→B and B→A) per feature (C, R).

Usage examples:
  python 14_aggregate_noninduced_v3.py --tag win160_k2_perm500_integrated --mode int \
      --combine stouffer --annotate_p --p_stride 6

Outputs (per tag → per monkey):
  manifest.csv, band_integrals.csv
  agg_{A}to{B}_{C|R}_{raw|int}.png/.csv               (single-direction figures + CSV)
  overlay_{C|R}_{A}_{B}_{raw|int}.png/.csv            (bidirectional overlay figures + CSV)
"""
...
OUT_ROOT  = BASE / "results" / "group_noninduced_pmeta"
...
def load_npz_noninduced(path: Path):
    ...
    rec = {
        ...
        # INT forward + null (sliding-integrated)
...
def combine_p_matrix(P: np.ndarray, method: str="stouffer") -> np.ndarray:
    """
    P: (S, T) per-session p-values. Combine across sessions per time.
    - stouffer: p = 1 - Phi( sum(Phi^{-1}(1-p_i))/sqrt(m) )
    - fisher:   p = 1 - CDF_chi2( -2 sum log p_i; df=2m )
    """
    S, T = P.shape
...
def plot_overlay_pair(x, res_ab, res_ba, mk, A, B, label, mode, out_png: Path,
    ...
    """
    res_* dicts: {"mu","lo","hi","p","sig"} aligned on time x.
    """
    fig, ax = plt.subplots(figsize=(10,4))
    # A->B
    ...
    # p annotations (offset to reduce overlap)
    if annotate_p:
...
def aggregate(tag: str, mode: str, out_dir: Path, combine: str,
    ...
    # Collect session-level series/nulls
    buckets = {"M": {}, "S": {}}
    manifests = {"M": [], "S": []}
    scalars   = {"M": [], "S": []}
    ...
    for sdir in sorted(SESS_ROOT.glob("*")):
        ...
        for npz in sorted(sub.glob("flow_timeseriesINT_*to*.npz")):
            ...
            scalars[mk].append({"sid": sid, "pair": f"{A}->{B}", "IC": rec["IC"], "pIC": rec["pIC"],
    ...
    for mk in ("M", "S"):
        ...
        # Storage for overlay later: store per feature per direction
        store = {"C": {}, "R": {}}  # store["C"]["A->B"] = dict(x,mu,lo,hi,p,sig)

        # Aggregate per ordered pair (single-direction figures + CSV), and cache for overlay
        ...
        for A in areas:
            for B in areas:
                ...
                # choose reference axis (prefer C)
                x_ref = None
        ...
        # --------- bidirectional overlays (for every unordered pair {A,B}) ---------
        for A in areas:
            for B in areas:
                if A >= B:  # avoid duplicates, require A<B lexicographically
                    continue
                for label in ("C","R"):
                    key_ab = f"{A}->{B}"
                    key_ba = f"{B}->{A}"
                    if key_ab in store[label] and key_ba in store[label]:
                        ...
                        # align on the AB time axis (assumes same grid; interpolate BA if needed)
                        x = res_ab["x"]
                        if not np.array_equal(res_ba["x"], x):
                            # interpolate BA to AB grid
                            def _interp(res):
                                def safe_interp(y): 
...
def main():
    ...
    ap.add_argument("--tag", required=True, help="subfolder ending with 'integrated' (e.g., win160_k2_perm500_integrated)")
    ...
    ap.add_argument("--combine", choices=["stouffer","fisher"], default="stouffer",
                    help="meta-analysis method to combine per-session p’s")
    ap.add_argument("--annotate_p", action="store_true", default=False)
    ap.add_argument("--p_stride", type=int, default=6, help="label every Nth point if annotate_p")
    args = ap.parse_args()
    ...
    print(f"[done] Non-induced p-meta aggregation + overlays ({args.mode}, {args.combine}) → {OUT}")
